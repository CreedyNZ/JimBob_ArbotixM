...
PAN_PROPORTIONAL_GAIN  =  -200
PAN_DERIVATIVE_GAIN    =  -300
TILT_PROPORTIONAL_GAIN =   500
TILT_DERIVATIVE_GAIN   =   600
...
def main():
  ...
  while run_flag:
    ...
    if (frame_index % 50) == 0:
      ...
      if count == 1:
        print '  sig:%2d x:%4d y:%4d width:%4d height:%4d pan:%1d tilt:%1d' % (block.signature, block.x, block.y, block.width, block.height, pan_gimbal.position, tilt_gimbal.position)
